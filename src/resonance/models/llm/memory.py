import os
import json
from .generation import generate_openrouter 
import env
...
def get_memories_w_tags(tags: list):
...
def get_memories_w_date(date: str, date_type=0) -> list:
    try:
        match date_type:
            case  2:
                query = """
...
def create_memories(context_target: str, ai_response="") -> bool:
    preprompt = """
    ...
    try: 
        if ai_response == "":
            ai_response = json.loads(generate_openrouter(context))
        else:
